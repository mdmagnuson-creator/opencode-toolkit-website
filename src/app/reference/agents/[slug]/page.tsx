...
// Get adjacent agents for prev/next navigation
// Order: Primary agents first, then subagents sorted alphabetically by name
function getAdjacentAgents(agent: Agent): { prev: Agent | null; next: Agent | null } {
  const sortedAgents = [...manifest.agents].sort((a, b) => {
    // Primary agents come first
    if (a.mode === "primary" && b.mode !== "primary") return -1;
    if (a.mode !== "primary" && b.mode === "primary") return 1;
    // Within same mode, sort alphabetically by name
    return a.name.localeCompare(b.name);
  });

  const currentIndex = sortedAgents.findIndex((a) => a.slug === agent.slug);
  
  return {
    prev: currentIndex > 0 ? sortedAgents[currentIndex - 1] : null,
    next: currentIndex < sortedAgents.length - 1 ? sortedAgents[currentIndex + 1] : null,
  };
}

// Generate breadcrumb items for the agent
function getBreadcrumbItems(agent: Agent) {
  const modeLabel = agent.mode === "primary" ? "Primary" : CATEGORY_LABELS[agent.category];
  
  return [
    { name: "Home", href: "/" },
    { name: "Reference", href: "/reference" },
    { name: "Agents", href: "/reference/agents" },
    { name: modeLabel, href: agent.mode === "primary" ? "/reference/agents/primary" : "/reference/agents/sub" },
    { name: agent.name, href: `/reference/agents/${agent.slug}` },
  ];
}
          ...
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              {relatedAgents.map((related) => (
                <Link
                  key={related.slug}
                  href={`/reference/agents/${related.slug}`}
        ...
        <section className="border-t border-neutral-200 px-6 py-8 sm:px-8 sm:py-10 lg:px-12 dark:border-neutral-800">
          <div className="mx-auto max-w-4xl">
            <nav className="flex items-stretch justify-between gap-4" aria-label="Agent navigation">
              ...
              {prev ? (
                <Link
                  href={`/reference/agents/${prev.slug}`}
              ...
              {next ? (
                <Link
                  href={`/reference/agents/${next.slug}`}
