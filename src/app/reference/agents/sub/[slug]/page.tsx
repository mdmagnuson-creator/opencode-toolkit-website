...
// Generate static params for subagents only
...
export function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  return params.then(({ slug }) => {
    const agent = manifest.agents.find((a) => a.slug === slug && a.mode === "subagent");
    if (!agent) {
...
function getRelatedAgents(agent: Agent): Agent[] {
  ...
  return manifest.agents
    .filter((a) => a.category === agent.category && a.slug !== agent.slug && a.mode === "subagent")
    .slice(0, 3);
}

// Get adjacent subagents for prev/next navigation (sorted alphabetically by name)
function getAdjacentAgents(agent: Agent): { prev: Agent | null; next: Agent | null } {
  const subAgents = manifest.agents
    .filter((a) => a.mode === "subagent")
    .sort((a, b) => a.name.localeCompare(b.name));

  const currentIndex = subAgents.findIndex((a) => a.slug === agent.slug);
  
  return {
    prev: currentIndex > 0 ? subAgents[currentIndex - 1] : null,
    next: currentIndex < subAgents.length - 1 ? subAgents[currentIndex + 1] : null,
  };
}
...
function getBreadcrumbItems(agent: Agent) {
  return [
    ...
    { name: "Sub-Agents", href: "/reference/agents/sub" },
    { name: agent.name, href: `/reference/agents/sub/${agent.slug}` },
  ];
}
